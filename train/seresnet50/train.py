...
# 构建SEresnet50网络, 这里直接用下载好的权重，所以pretrained=None
se_resnet50_ = se_resnet50(num_classes=NUM_CATEGORIES, pretrained=None)
# 加上strict=False表示忽略掉模型和参数文件中不匹配的参数
pre_weights = torch.load(WEIGHT_PATH)
pre_weights.pop('fc.weight')
pre_weights.pop('fc.bias')
se_resnet50_.load_state_dict(pre_weights, strict=False)
model = torch.nn.DataParallel(se_resnet50_)  # 多GPU并行计算
