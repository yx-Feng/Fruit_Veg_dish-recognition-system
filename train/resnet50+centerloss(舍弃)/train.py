...
# Constants常量
# ==================================================================
EPOCH = 5                  # epoch的数量，也就是整个数据集训练的次数
...
# 构建网络
se_resnet50_cbam = ResidualNet(50, NUM_CATEGORIES, use_cbam=False)
pre_weights = torch.load(WEIGHT_PATH)
pre_weights.pop('fc.weight')
pre_weights.pop('fc.bias')
se_resnet50_cbam.load_state_dict(pre_weights, strict=False)
model = torch.nn.DataParallel(se_resnet50_cbam)  # 多GPU并行计算
...
# CenterLoss
loss_weight = 0
centerloss = CenterLoss(NUM_CATEGORIES, 2).to(device)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # 计算输出
        ip1, output = model(input_var)
        print(output)
        print(output.shape)
        print(target_var)
        print(target_var.shape)
        # loss函数
        loss = nllloss(output, target_var) + loss_weight * centerloss(target_var, ip1)
        # 计算accuracy, 记录loss
...
for epoch in range(0, EPOCH):
    ...
    # 训练一个epoch
    train_loss, train_top1, train_top5 = train(train_loader, model, optimizer, epoch+1)
    # 验证集评价
    ...
    data.to_csv('./loss_acc/train_val.csv', mode='a', header=False, index=False)  # mode设为a,就可以向csv文件追加数据了
    # 记录最好的prec@1
