...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, use_cbam=False):
        self.inplanes = 64
        ...
        # 第一个参数表示输入的神经元个数、第二个参数表示输出的神经元个数，Linear就是对输入做了一个线性变换
        self.ip1 = nn.Linear(512 * block.expansion, 2)
        self.ip2 = nn.Linear(2, num_classes, bias=False)

        init.kaiming_normal(self.ip1.weight)
        init.kaiming_normal(self.ip2.weight)
...
def ResidualNet(depth, num_classes, use_cbam):

    assert depth in [18, 34, 50, 101], 'network depth should be 18, 34, 50 or 101'

    if depth == 18:
        model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes, use_cbam)

    elif depth == 34:
        model = ResNet(BasicBlock, [3, 4, 6, 3], num_classes, use_cbam)

    elif depth == 50:
        model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes, use_cbam)

    elif depth == 101:
        model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes, use_cbam)
